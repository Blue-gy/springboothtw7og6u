	...
	<div class="addEdit-block" :style='{"width":"100%","padding":"50px 0 30px 0","margin":"0","overflow-y":"auto","background":"#fff","height":"auto"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="库存数量" prop="kucunshuliang">
					<el-input v-model.number="ruleForm.kucunshuliang" placeholder="库存数量" clearable  :readonly="ro.kucunshuliang"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="库存数量" prop="kucunshuliang">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.shebeijianjie" label="设备简介" prop="shebeijianjie">
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `shebeizulin/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shebeizulinCrossAddOrUpdateFlag = false;
											this.parent.search();
