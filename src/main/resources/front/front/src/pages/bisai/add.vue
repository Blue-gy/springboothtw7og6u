...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="120px"
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="赛事名称" prop="saishimingcheng">
            <el-input v-model="ruleForm.saishimingcheng" 
                placeholder="赛事名称" clearable :disabled=" false  ||ro.saishimingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="比赛时间" prop="bisaishijian">
            <el-input v-model="ruleForm.bisaishijian" 
                placeholder="比赛时间" clearable :disabled=" false  ||ro.bisaishijian"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="比赛地点" prop="bisaididian">
            <el-input v-model="ruleForm.bisaididian" 
                placeholder="比赛地点" clearable :disabled=" false  ||ro.bisaididian"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="比赛图片" v-if="type!='cross' || (type=='cross' && !ro.bisaitupian)" prop="bisaitupian">
            <file-upload
            tip="点击上传比赛图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' class="upload" v-else label="比赛图片" prop="bisaitupian">
                <img v-if="ruleForm.bisaitupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.bisaitupian.split(',')[0]" width="100" height="100">
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.bisaitupian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`bisai/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('bisai/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`bisai/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
