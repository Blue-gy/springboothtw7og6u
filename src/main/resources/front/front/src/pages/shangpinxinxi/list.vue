<template>
  <div>
    <div
        :style='{"padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.1)","margin":"0px auto","borderColor":"#75664D","background":"#fff","borderWidth":"5px 0 0 0","width":"80%","borderStyle":"solid"}'
        class="breadcrumb-preview">
      <el-breadcrumb :separator="'>'"
                     :style='{"fontSize":"14px","lineHeight":"1","justifyContent":"center","display":"flex"}'>
    ...
    <div class="list-preview"
         :style='{"padding":"0 10% 20px","margin":"20px auto","flexWrap":"wrap","background":"#fff","flexDirection":"row","display":"flex","width":"100%","position":"relative"}'>


      <el-form :inline="true" :model="formSearch" class="list-form-pv"
               :style='{"border":"1px solid #C7C7C7","padding":"20px 10px 0","margin":"0 0 20px","alignItems":"center","background":"#fff","flexDirection":"row","display":"flex","width":"100%","height":"auto","order":"1"}'>
        <el-form-item :style='{"flexWrap":"wrap","margin":"0 0 20px","display":"flex"}'>
          <div class="lable" v-if="true"
               :style='{"width":"auto","padding":"0 5px 0","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>
            商品名称：
          </div>
          <el-input v-model="formSearch.shangpinmingcheng" placeholder="商品名称"
        ...
        <el-form-item :style='{"flexWrap":"wrap","margin":"0 0 20px","display":"flex"}'>
          ...
          <el-input v-model="formSearch.pinpai" placeholder="品牌" @keydown.enter.native="getList(1, curFenlei)"
                    clearable></el-input>
        </el-form-item>
        <el-form-item :style='{"flexWrap":"wrap","margin":"0 0 20px","display":"flex"}'>
          <div class="lable" v-if="true"
          ...
          <el-input v-model="formSearch.guige" placeholder="规格" @keydown.enter.native="getList(1, curFenlei)"
                    clearable></el-input>
        </el-form-item>
        <el-form-item :style='{"flexWrap":"wrap","margin":"0 0 20px","display":"flex"}'>
          <div class="lable" v-if="true"
        ...
        <el-button v-if=" true "
                   :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 5px 20px","outline":"none","color":"#fff","borderRadius":"0px","background":"#75664d","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'
        ...
        <el-button v-if="btnAuth('shangpinxinxi','新增')"
                   :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 0 20px","outline":"none","color":"#fff","borderRadius":"0px","background":"#75664d","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'
      ...
      <div class="select2"
           :style='{"width":"100%","padding":"0","margin":"0 0 10px","background":"#fff","height":"auto"}'>
        <div
            :style='{"border":"1px solid #ccc","padding":"4px 20px","margin":"4px 0 10px","background":"#fff","width":"100%","position":"relative","边框":"","height":"auto"}'
            class="list" v-for="(item,index) in selectOptionsList" :key="item">
          <div
              :style='{"padding":"0 5px","lineHeight":"32px","fontSize":"14px","color":"#75664d","display":"inline-block"}'
          ...
          <div :style='{"width":"auto","flexWrap":"wrap","display":"inline-block","height":"auto"}' class="item-body">
            ...
            <div class="item" @click="selectClick2(item,index1)" :class="item.check == index1 ? 'active' : ''"
                 v-for="item1,index1 in item.list" :key="item1">{{ item1 }}
            </div>
          </div>
        </div>
      </div>
      <div class="sort_view"
           :style='{"border":"1px solid #C7C7C7","width":"100%","margin":"0 0 20px","textAlign":"right"}'>
        <el-button :style='{"border":"0","padding":"0 15px","margin":"0 5px","borderRadius":"8px"}'
                   @click="sortClick('price')">
          <span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#333"}'
                class="icon iconfont icon-xiaoliang17" v-if="sortType!='price'"></span>
          <span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#333"}'
                class="icon iconfont icon-xiaoliang17" v-else-if="sortType=='price'&&sortOrder=='desc'"></span>
          <span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#333"}'
                class="icon iconfont icon-xiaoliang17" v-else-if="sortType=='price'&&sortOrder=='asc'"></span>
        ...
        <el-button :style='{"border":"0","padding":"0 15px","margin":"0 5px","borderRadius":"8px"}'
          ...
          <span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#333"}'
                class="icon iconfont icon-xiaoliang13" v-else-if="sortType=='clicknum'&&sortOrder=='desc'"></span>
          <span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#333"}'
                class="icon iconfont icon-xiaoliang13" v-else-if="sortType=='clicknum'&&sortOrder=='asc'"></span>
          <span :style='{"color":"#333","lineHeight":"40px","fontSize":"14px"}'>点击量</span>
        </el-button>
      </div>
      <div class="list"
           :style='{"border":"1px solid #C7C7C7","width":"auto","margin":"0","background":"#fff","flex":"1","order":"3"}'>
        <!-- 样式一 -->
        <div class="list1 index-pv1"
             :style='{"width":"100%","padding":"20px 10px 0","background":"#fff","height":"auto"}'>
          <div
              :style='{"border":"1px solid #E0E0E0","padding":"8px","margin":"0 1% 20px","background":"#ffff","display":"inline-block","width":"23%","position":"relative","height":"auto"}'
              v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)"
              class="list-item animation-box">
            <img @click.stop="imgPreView(item.shangpintupian)"
                 :style='{"width":"100%","objectFit":"cover","display":"block","height":"150px"}'
                 v-if="item.shangpintupian && item.shangpintupian.substr(0,4)=='http'" :src="item.shangpintupian.split(',')[0]"
                 class="image"/>
            <img @click.stop="imgPreView(baseUrl + (item.shangpintupian?item.shangpintupian.split(',')[0]:''))"
                 :style='{"width":"100%","objectFit":"cover","display":"block","height":"150px"}' v-else
                 :src="baseUrl + (item.shangpintupian?item.shangpintupian.split(',')[0]:'')" class="image"/>
            <div v-if="item.price"
                 :style='{"padding":"0 10px","lineHeight":"30px","fontSize":"14px","color":"red","textAlign":"center"}'
                 class="price"><span :style='{"fontSize":"12px"}'>￥</span>{{ item.price }}
            </div>
            <div :style='{"padding":"0 10px","lineHeight":"1.5","fontSize":"14px","color":"#000","textAlign":"center"}'
            ...
            <div :style='{"padding":"0 10px","display":"none"}'>
              ...
              <span class="text"
                    :style='{"color":"#666","lineHeight":"1.5","fontSize":"12px"}'>{{ item.addtime }}</span>
            </div>
            <div :style='{"padding":"0 10px","display":"none"}'>
              <span class="icon iconfont icon-shoucang10"
                    :style='{"margin":"0 2px 0 0","lineHeight":"1.5","fontSize":"12px","color":"#666"}'></span>
              <span class="text"
                    :style='{"color":"#666","lineHeight":"1.5","fontSize":"12px"}'>{{ item.storeupnum }}</span>
            </div>
            <div :style='{"padding":"0 10px","display":"none"}' v-if="item.clicknum">
              <span class="icon iconfont icon-chakan9"
                    :style='{"margin":"0 2px 0 0","lineHeight":"1.5","fontSize":"12px","color":"#666"}'></span>
              <span class="text"
                    :style='{"color":"#666","lineHeight":"1.5","fontSize":"12px"}'>{{ item.clicknum }}</span>
            </div>
          </div>
        </div>
    ...
    this.$http.get('option/shangpinfenlei/shangpinfenlei').then(res => {
      if (res.data.code == 0) {
        ...
        this.selectOptionsList.push({
          name: '商品分类',
          list: this.shangpinfenleiOptions,
