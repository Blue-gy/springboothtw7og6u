...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="120px"
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="设备名称" prop="shebeimingcheng">
            <el-input v-model="ruleForm.shebeimingcheng" 
                placeholder="设备名称" clearable :disabled=" false  ||ro.shebeimingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="租赁数量" prop="kucunshuliang">
            <el-input v-model.number="ruleForm.kucunshuliang" 
                placeholder="租赁数量" clearable :disabled=" false  ||ro.kucunshuliang"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="设备图片" v-if="type!='cross' || (type=='cross' && !ro.shebeitupian)" prop="shebeitupian">
            <file-upload
            tip="点击上传设备图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' class="upload" v-else label="设备图片" prop="shebeitupian">
                ...
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.shebeitupian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="设备简介" prop="shebeijianjie">
            <el-input v-model="ruleForm.shebeijianjie" 
                placeholder="设备简介" clearable :disabled=" false  ||ro.shebeijianjie"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="租赁价格" prop="zulinjiage">
            <el-input v-model.number="ruleForm.zulinjiage" 
                placeholder="租赁价格" clearable :disabled=" false  ||ro.zulinjiage"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="租赁金额" prop="zulinjine">
              <el-input v-model="zulinjine" placeholder="租赁金额" disabled></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="租赁时间" prop="zulinshijian">
              <el-date-picker
...
<script>
  export default {
    ...
    computed: {
        ...
        zulinjine:{
            get: function () {
                return 1*this.ruleForm.kucunshuliang*this.ruleForm.zulinjiage
            }
        },
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`zulinjilu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('zulinjilu/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								obj.kucunshuliang = parseFloat(obj.kucunshuliang) - parseFloat(this.ruleForm.kucunshuliang)

								this.$http.post(table+`/update`,obj).then(res => {});
								this.$http.post(`zulinjilu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
